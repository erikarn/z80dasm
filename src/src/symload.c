...
static char *include[MAX_INCLUDE_TOP];
static int includenum=0;

static char *comments[MAX_COMMENT_LINES_PER_SYMBOL];
static long commentsnum = 0;
...
static void symbol_free_comments() {
	int n;
...
static char *symbol_comment_string() {
	long bytes_needed;
	char *single_string_comment;
	long pointer;
	int i;
	int line_size;
	...
	for (i=0; i<commentsnum; i++) {
		bytes_needed += strlen(comments[i]);
	}
	single_string_comment = malloc(bytes_needed);

	pointer = 0;
	for (i=0; i<commentsnum; i++) {
		line_size = strlen(comments[i]);
		strcpy(single_string_comment + pointer, comments[i]);
		pointer += line_size;
	}

	return single_string_comment;
}
...
int symbol_load_file(char *filename, int level)
{
	...
	while(fgets(line, MAX_LINE_LEN, f)!=NULL) {
		...
		if(r==1) {
			comment = symbol_comment_string();
			symbol_new(name, val, comment, 50, level!=0);
			free(name);
			free(comment);
			symbol_free_comments();
		}
...
int symbol_load_line(char *line, char **name, int *val, int level)
{
	...
	if(line[0]==';') {
		/* Comments on their own line are collected, to be possibly
		 * included with the symbol definition. */

		if (a_symcomments) {
			if (commentsnum >= MAX_COMMENT_LINES_PER_SYMBOL) {
				msg(0, "Maximum number of comments exceeded\n");
				return -1;
			}
			comments[commentsnum] = strdup(line);
			commentsnum++;
		}
		return 0;
	} else {
		/* Comments appended at the end of the line are ignored. */

		linec=strtok(line, ";");
	}

	/* Collected comments are flushed on an empty line. */
	if(linec==NULL) {
		/* empty line */
		symbol_free_comments();
		return 0;
